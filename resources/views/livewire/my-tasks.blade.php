    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('user.dashboard') }}" wire:navigate class="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition-all duration-300 flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-zinc-900 dark:text-white">{{ $stats['total_active'] }}</p>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $stats['pending_verification'] }}</p>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700 hover:shadow-lg transition-all duration-300">
                ...
                <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ $stats['near_deadline'] }}</p>
            </div>
        </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="updateFilter('all')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'all' ? 'bg-green-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
                        ...
                        <button wire:click="updateFilter('taken')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'taken' ? 'bg-orange-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
                        ...
                        <button wire:click="updateFilter('pending')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'pending' ? 'bg-purple-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
                        ...
                        <button wire:click="updateFilter('near_deadline')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $filterStatus === 'near_deadline' ? 'bg-red-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button wire:click="updateSort('deadline')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $sortBy === 'deadline' ? 'bg-green-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
                        ...
                        <button wire:click="updateSort('status')" 
                            ...
                            class="px-4 py-2 rounded-lg text-sm transition-all {{ $sortBy === 'status' ? 'bg-green-600 text-white' : 'bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-600' }}">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden transition-all duration-300 flex flex-col {{ $isForfeited ? 'opacity-60 grayscale ring-2 ring-red-500 bg-red-50 dark:bg-red-950/20' : 'hover:shadow-xl group' }} {{ !$isForfeited && $isCritical ? 'ring-2 ring-red-500/70' : '' }} {{ !$isForfeited && $isVeryUrgent && !$isCritical ? 'ring-2 ring-orange-500/50' : '' }} {{ !$isForfeited && $isUrgent && !$isVeryUrgent && !$isCritical ? 'ring-2 ring-yellow-500/50' : '' }}">
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-2 flex-wrap">
                                <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $statusColor }}-100 dark:bg-{{ $statusColor }}-900/30 text-{{ $statusColor }}-700 dark:text-{{ $statusColor }}-400">
                                    {{ $statusLabel }}
                                </span>
                                @if($task->category)
                ...
                <div class="p-5 flex-1">
                    <div class="space-y-3 mb-4">
                        <!-- Deadline Info -->
                        @if($userTask->deadline_at)
                        <div class="flex items-center justify-between text-sm {{ $isOverdue || $isCritical || $isVeryUrgent ? 'bg-red-50 dark:bg-red-950/30 -mx-5 px-5 py-2' : '' }}">
                            <div class="flex items-center gap-2 {{ $isOverdue || $isCritical ? 'text-red-600 dark:text-red-400' : ($isVeryUrgent ? 'text-orange-600 dark:text-orange-400' : ($isUrgent ? 'text-yellow-600 dark:text-yellow-400' : 'text-zinc-600 dark:text-zinc-400')) }}">
                            ...
                            <div class="text-right">
                                <div class="font-semibold {{ $isOverdue || $isCritical ? 'text-red-600 dark:text-red-400' : ($isVeryUrgent ? 'text-orange-600 dark:text-orange-400' : ($isUrgent ? 'text-yellow-600 dark:text-yellow-400' : 'text-zinc-900 dark:text-white')) }}">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full max-w-2xl mx-auto my-4 overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-5 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-green-100 text-xs">{{ $selectedTaskData->category?->name ?? 'Uncategorized' }}</p>
                    </div>
                </div>
            ...
            <div class="p-5 sm:p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <span class="px-3 py-1 rounded-full text-xs font-medium 
                            @if($selectedUserTask->status === 'taken') bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400
                            @elseif(in_array($selectedUserTask->status, ['pending_verification_1', 'pending_verification_2'])) bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400
                ...
                <div class="bg-zinc-50 dark:bg-zinc-900/50 rounded-xl p-4">
                    ...
                    <p class="text-sm text-zinc-600 dark:text-zinc-400 whitespace-pre-line">{{ $selectedTaskData->description }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
                        ...
                        <p class="text-sm font-bold text-amber-800 dark:text-amber-200">{{ $selectedUserTask->deadline_at->format('d M Y, H:i') }}</p>
                        <p class="text-xs text-amber-600 dark:text-amber-400">{{ $selectedUserTask->deadline_at->diffForHumans() }}</p>
                    </div>
                    @endif
                    ...
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3">
                        ...
                        <p class="text-sm font-bold text-green-800 dark:text-green-200">Rp {{ number_format($selectedUserTask->payment_amount, 0, ',', '.') }}</p>
                        @elseif($selectedTaskData->estimated_amount)
                        <p class="text-sm font-bold text-green-800 dark:text-green-200">Rp {{ number_format($selectedTaskData->estimated_amount, 0, ',', '.') }}</p>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-3">
                        ...
                        <p class="text-sm font-bold text-blue-800 dark:text-blue-200">{{ $selectedUserTask->taken_at->format('d M Y, H:i') }}</p>
                    </div>
                    ...
                    <div class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-3">
                        ...
                        <a href="{{ $selectedTaskData->whatsapp_group_link }}" target="_blank" class="text-sm font-bold text-emerald-800 dark:text-emerald-200 hover:underline truncate block">
                            Buka Link WhatsApp →
                        </a>
                    </div>
                    @endif
