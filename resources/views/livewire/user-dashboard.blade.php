            ...
            <div class="relative overflow-hidden bg-gradient-to-r from-green-600 via-green-700 to-emerald-700 dark:from-green-900 dark:via-green-800 dark:to-emerald-900 rounded-2xl p-6 shadow-xl">
                ...
                <div class="relative flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-xl md:text-2xl font-bold text-white mb-1">Ada {{ $stats['available_tasks'] }} Tugas Baru Menunggu!</h3>
                            <p class="text-green-100 text-sm md:text-base">Ambil tugas sekarang dan mulai hasilkan uang 💰</p>
                        </div>
                    </div>
                    <a href="{{ route('user.dashboard') }}" class="flex-shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-white hover:bg-green-50 text-green-700 font-bold rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-2xl group">
                ...
                <div class="relative overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
                    ...
                    <div class="flex items-start justify-between relative">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-blue-600 dark:text-blue-400" x-data="{ count: 0 }" x-init="setTimeout(() => { let target = {{ $stats['total_tasks'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 100)">
                ...
                <div class="relative overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
                    ...
                    <div class="flex items-start justify-between relative">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-orange-600 dark:text-orange-400" x-data="{ count: 0 }" x-init="setTimeout(() => { let target = {{ $stats['in_progress'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 200)">
                                <span x-text="Math.floor(count)">0</span>
                            </h3>
                            <div class="flex items-center mt-2">
                                <a href="{{ route('user.history') }}?filter=my_tasks" class="text-green-600 dark:text-green-400 text-xs hover:underline">
                                    Lihat Tugas →
                                </a>
                            </div>
                        </div>
                ...
                <div class="relative overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
                    ...
                    <div class="flex items-start justify-between relative">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-green-600 dark:text-green-400" x-data="{ count: 0 }" x-init="setTimeout(() => { let target = {{ $stats['completed'] }}; let increment = target / 50; let timer = setInterval(() => { count += increment; if (count >= target) { count = target; clearInterval(timer); } }, 20); }, 300)">
                                <span x-text="Math.floor(count)">0</span>
                            </h3>
                            <div class="flex items-center mt-2">
                                <a href="{{ route('user.history') }}?filter=completed" class="text-green-600 dark:text-green-400 text-xs hover:underline">
                                    Lihat Selesai →
                                </a>
                            </div>
                        </div>
                ...
                <div class="relative overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 group">
                    ...
                    <div class="flex items-start justify-between relative">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-amber-600 dark:text-amber-400" x-data="{ amount: 0 }" x-init="setTimeout(() => { let target = {{ $stats['total_earnings'] }}; let increment = target / 50; let timer = setInterval(() => { amount += increment; if (amount >= target) { amount = target; clearInterval(timer); } }, 20); }, 400)">
            ...
            <div class="relative flex-1 overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-4 md:p-6">
                ...
                <div x-show="activeTab === 'overview'" class="h-full">
                            ...
                            @foreach($weeklyActivity as $day)
                                ...
                                <div class="flex-1 flex flex-col items-center group" x-data="{ animated: false }" x-intersect="animated = true">
                                    <div class="w-full relative h-48">
                                        <div class="w-full absolute bottom-0 rounded-t-lg transition-all duration-1000 ease-out {{ $day['count'] > 0 ? 'bg-gradient-to-t from-green-500 to-green-600' : 'bg-zinc-200 dark:bg-zinc-700' }}"
                                    ...
                                    <span class="text-xs text-zinc-600 dark:text-zinc-400 mt-2">{{ $day['day'] }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('user.history') }}" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">Lihat Semua</a>
                    </div>
                    <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-900/30 rounded-lg transition-all duration-300 hover:bg-zinc-100 dark:hover:bg-zinc-900/50 hover:shadow-md">
                                <div class="flex items-center gap-4 flex-1 min-w-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h4 class="font-medium text-zinc-900 dark:text-white truncate">{{ $userTask->task->title }}</h4>
                                ...
                                <div class="flex items-center gap-3 flex-shrink-0">
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold
                                               @if($userTask->status === 'taken') bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400
                                               @elseif($userTask->status === 'pending_verification_1' || $userTask->status === 'pending_verification_2') bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400
                                               @elseif($userTask->status === 'completed') bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                                               @elseif($userTask->status === 'failed') bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400
                                    ...
                                    @if($userTask->isOverdue())
                                        <span class="inline-flex items-center gap-2 px-2 py-1 rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 text-xs font-semibold">❌ Gagal (Kadaluarsa)</span>
                                    @else
                                        <a href="{{ route('user.task.work', $userTask->task_id) }}" class="text-zinc-600 dark:text-zinc-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                        ...
                        @empty
                            <div class="text-center py-12">
                                ...
                                <a href="{{ route('user.dashboard') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors">
                            ...
                            <div class="space-y-4">
                                <div x-data="{ animated: false }" x-intersect="animated = true">
                                    <div class="flex justify-between text-sm mb-1">
                                        ...
                                        <span class="font-medium text-green-600 dark:text-green-400">{{ $paymentStats['paid'] }}</span>
                                    </div>
                                    <div class="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-3">
                                        <div class="bg-gradient-to-r from-green-500 to-green-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                             :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['paid'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'"></div>
                                    </div>
                                </div>
                                <div x-data="{ animated: false }" x-intersect="animated = true">
                                    <div class="flex justify-between text-sm mb-1">
                                        <span class="text-zinc-700 dark:text-zinc-300">Menunggu</span>
                                        <span class="font-medium text-yellow-600 dark:text-yellow-400">{{ $paymentStats['pending'] }}</span>
                                    </div>
                                    <div class="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-3">
                                        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                             :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['pending'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'"></div>
                                    </div>
                                </div>
                                <div x-data="{ animated: false }" x-intersect="animated = true">
                                    <div class="flex justify-between text-sm mb-1">
                                        <span class="text-zinc-700 dark:text-zinc-300">Gagal</span>
                                        <span class="font-medium text-red-600 dark:text-red-400">{{ $paymentStats['failed'] }}</span>
                                    </div>
                                    <div class="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-3">
                                        <div class="bg-gradient-to-r from-red-500 to-red-600 h-3 rounded-full transition-all duration-1000 ease-out"
                                             :style="animated ? 'width: {{ $stats['completed'] > 0 ? min(100, ($paymentStats['failed'] / $stats['completed']) * 100) : 0 }}%' : 'width: 0%'"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-900/30 rounded-xl p-5 border border-zinc-200 dark:border-zinc-700 transition-all duration-300 hover:shadow-md md:col-span-2">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700">
                                    ...
                                    <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['available_tasks'] }}</p>
                                </div>
                                <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700">
                                ...
                                <div class="bg-white dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700">
                                    ...
                                    <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">
                                        Rp {{ $stats['completed'] > 0 ? number_format($stats['total_earnings'] / $stats['completed'], 0, ',', '.') : 0 }}
                                    </p>
                                </div>
                            </div>
                        </div>
